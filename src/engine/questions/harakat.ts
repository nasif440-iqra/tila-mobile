...
interface HarakatDifficulty {
  isHeavy: boolean;
  isThroat: boolean;
  isMastery: boolean;
  isEarlyHarakat: boolean;
}

function resolveHarakatCombos(lesson: HarakatLesson): HarakatCombo[] {
  if (lesson.teachCombos?.length) {
    return lesson.teachCombos.map(id => getCombo(id)).filter(Boolean) as HarakatCombo[];
  }
  return generateHarakatCombos(lesson.teachIds || [], lesson.teachHarakat);
}

function getHarakatDifficulty(lesson: HarakatLesson): HarakatDifficulty {
...
export function generateHarakatIntroQs(lesson: HarakatLesson): Question[] {
  const harakat = (lesson.teachHarakat || []).map(id => getHarakah(id)).filter(Boolean) as Harakah[];
  const qs: Question[] = [];
...
export function generateHarakatQs(lesson: HarakatLesson): Question[] {
  ...
  if (isMixed) {
    ...
    const byLetter: Record<number, HarakatCombo[]> = {};
    ...
    if ((diff.isHeavy || diff.isThroat) && letterGroups.length <= 2) {
      ...
      phaseA.push({ type: "tap", prompt: `Which one says \u201C${warmupTarget.sound}\u201D?`, targetId: warmupTarget.id, isHarakat: true, hasAudio: true, options: shuffle(warmupOpts.map(c => ({ id: c.id, label: c.display, isCorrect: c.id === warmupTarget.id }))) });
      const fullGroup = letterGroups.length > 1 ? letterGroups[1] : firstGroup;
      const fullTarget = pickRandom(fullGroup)!;
      phaseA.push({ type: "tap", prompt: `Which one says \u201C${fullTarget.sound}\u201D?`, targetId: fullTarget.id, isHarakat: true, hasAudio: true, options: shuffle(fullGroup.map(c => ({ id: c.id, label: c.display, isCorrect: c.id === fullTarget.id }))) });
    } else {
      for (const letterCombos of letterGroups.slice(0, 3)) {
        const target = pickRandom(letterCombos)!;
        phaseA.push({ type: "tap", prompt: `Which one says \u201C${target.sound}\u201D?`, targetId: target.id, isHarakat: true, hasAudio: true, options: shuffle(letterCombos.map(c => ({ id: c.id, label: c.display, isCorrect: c.id === target.id }))) });
      }
    }

    // Phase B
    const phaseB: Question[] = [];
    ...
    for (const c of shuffle([...combos]).slice(0, phaseBCount)) {
      ...
      phaseB.push({ type: "letter_to_name", prompt: c.display, promptSubtext: "What sound does this make?", targetId: c.id, isHarakat: true, options: shuffle(sameLetter.map(cc => ({ id: cc.id, label: `\u201C${cc.sound}\u201D`, isCorrect: cc.id === c.id }))) });
    }

    // Phase C
    const phaseC: Question[] = [];
    const letterIds = Object.keys(byLetter);
    if (letterIds.length >= 2) {
      const crossCount = diff.isMastery ? 2 : 1;
      const usedHarakat = new Set<string>();
    ...
    // Phase D
    const phaseD: Question[] = [];
    if (lesson.reviewIds?.length > 0) {
      const reviewCount = diff.isMastery ? 2 : 1;
      const reviewCombos = generateHarakatCombos(lesson.reviewIds) as HarakatCombo[];
      const usedReviewLetters = new Set<number>();
      for (let i = 0; i < reviewCount && reviewCombos.length > 0; i++) {
        ...
        phaseD.push({ type: "tap", prompt: `Review: which says \u201C${target.sound}\u201D?`, targetId: target.id, isHarakat: true, hasAudio: true, options: shuffle(sameLetter.map(c => ({ id: c.id, label: c.display, isCorrect: c.id === target.id }))) });
      }
    }
  ...
  for (const c of combos) {
    qs.push({ type: "tap", prompt: `Which one says \u201C${c.sound}\u201D?`, targetId: c.id, isHarakat: true, hasAudio: true, options: shuffle(combos.map(cc => ({ id: cc.id, label: cc.display, isCorrect: cc.id === c.id }))) });
  }
  for (const c of combos) {
    qs.push({ type: "letter_to_name", prompt: c.display, promptSubtext: "What sound does this make?", targetId: c.id, isHarakat: true, options: shuffle(combos.map(cc => ({ id: cc.id, label: `\u201C${cc.sound}\u201D`, isCorrect: cc.id === c.id }))) });
  }
  return qs;
}

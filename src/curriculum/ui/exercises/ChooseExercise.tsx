...
import type { ChooseExercise as ChooseExerciseData } from "../../types";
import type { EntityAttempt, ScreenOutcome } from "../../runtime/LessonRunner";

interface Props {
  screenId: string;
  exercise: ChooseExerciseData;
...
export function ChooseExercise({
  ...
  const handlePlayAudio = () => {
    if (!exercise.audioPrompt) return;
    setPlaying(true);
    onPlayAudio?.(exercise.audioPrompt);
    if (audioTimerRef.current) clearTimeout(audioTimerRef.current);
    audioTimerRef.current = setTimeout(() => setPlaying(false), AUDIO_FLASH_MS);
  };
  ...
  const handleTap = (optionIndex: number) => {
    ...
    if (opt.correct) {
      setLocked(true);
      setOptionStates(
        exercise.options.map((_, i) => (i === optionIndex ? "correct" : "dim"))
      );
    ...
    wrongTimerRef.current = setTimeout(() => {
      setOptionStates((prev) =>
        prev.map((s, i) => (i === optionIndex ? "idle" : s))
      );
    }, WRONG_FEEDBACK_MS);
  };

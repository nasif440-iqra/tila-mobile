...
function StaticWarmGlow({
  ...
  return (
    <View
      ...
      pointerEvents="none"
    >
      <GlowSvg size={size} opacity={opacity} color={color} glowId={glowId} />
    </View>
  );
}
...
function AnimatedWarmGlow({
  ...
  useEffect(() => {
    ...
    pulseOpacity.value = withRepeat(
      withSequence(
        withTiming(pulseMax, { duration: breathing.inhale, easing: Easing.inOut(Easing.ease) }),
        withTiming(pulseMin, { duration: breathing.exhale, easing: Easing.inOut(Easing.ease) }),
      ),
      -1,
      false,
    );
    pulseScale.value = withRepeat(
      withSequence(
        withTiming(breathing.scaleMax, { duration: breathing.inhale, easing: Easing.inOut(Easing.ease) }),
        withTiming(breathing.scaleMin, { duration: breathing.exhale, easing: Easing.inOut(Easing.ease) }),
      ),
      -1,
      false,
    );
  ...
  return (
    <Animated.View
      ...
      pointerEvents="none"
    >
      <GlowSvg size={size} opacity={baseOpacity * 2} color={color} glowId={glowId} />
    </Animated.View>
  );
}
